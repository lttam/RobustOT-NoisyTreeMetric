...
distanceName = 'RT_Local'; %l2 cost matrix
...
for iiTree = 1:nTree
    ...
    % Compute UU_Edge (upper bound of edge weights for robust TW)
    UU_Edge = WW_Edge_noise + alphaInterval;

    disp('...Computing l1-distance for tree representation data');
    
    tic    
    % for robust TW (interval)
    XX_TMWW = XX_TM .* UU_Edge; % noise on L2 distance (edge)
    ...
    for ii = 1:(length(YY)-1)
        ...
        DD_XX(ii, (ii+1):length(YY)) = tmp';
        DD_XX((ii+1):length(YY), ii) = tmp;
    end
    runTime_TW = toc;
